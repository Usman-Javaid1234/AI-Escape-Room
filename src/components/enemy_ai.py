...
class EnemyAI:
    def __init__(self):
        ...
        self.aggro_range = 120       # Increased range to start chase
        self.chase_range = 80   
        self.walk_speed = 18
        ...
        self.patrol_index = 0  # Add this for patrol functionality
    ...
    def decide_action(self):
        ...
        if self.state == "find_lever":
            # First try to find an active lever to deactivate
            if not self.find_next_lever():
                # If no active levers found, continue patrolling
    ...
    def patrol_lever_positions(self):
        ...
        if math.sqrt(dx * dx + dy * dy) < 50:
            ...
            next_pos = self.known_lever_positions[self.patrol_index]
            self.target = (next_pos[0] * 32, next_pos[1] * 32)
            print(f"Moving to next patrol point: {next_pos}")
    ...
    def find_target(self):
        ...
        if min_distance > self.chase_range:
            self.in_chase = False
            self.target = None  # Clear the target position when target escapes
            self.targeted_entity = None
